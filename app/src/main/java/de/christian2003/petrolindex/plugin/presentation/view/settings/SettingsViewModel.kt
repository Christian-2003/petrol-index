...
class SettingsViewModel: ViewModel() {
    ...
    fun restoreDataFromJsonFile(uri: Uri, onFinished: (Boolean) -> Unit) = viewModelScope.launch {
        /* TODO: Move import to use case
        val gson = Gson()
        var petrolEntries: Array<ConsumptionEntity>? = null
        val json = readFromFile(uri)
        var success = false
        if (json != null) {
            try {
                petrolEntries = gson.fromJson(json, Array<ConsumptionEntity>::class.java)
            }
            catch (e: Exception) {
